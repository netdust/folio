 ...
 *
 * Hidden by the parent when doc.type !== 'agent' (the server's
 * INVALID_ACTIVITY_TARGET rejects triggers; we don't want to render a button
 * that always errors).
 */
interface Props {
  wslug: string;
...
export function WorkspaceLogActivityButton({ wslug, slug }: Props) {
  const [open, setOpen] = useState(false);
  const [note, setNote] = useState('');
  const log = useWorkspaceLogActivity(wslug);
