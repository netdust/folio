 ...
 *
 * Same visual surface as the project-scoped panel — only the events source is
 * different (workspace endpoint, no pslug). Kept as a separate component
 * rather than threading an optional pslug through the project version, per the
 * "project vs workspace" split convention used elsewhere in the codebase.
 */
interface Props {
  wslug: string;
  slug: string;
...
export function WorkspaceActivityPanel({ wslug, slug }: Props) {
  const [expanded, setExpanded] = useState(true);
  const { data: events, isLoading } = useWorkspaceDocumentEvents(wslug, slug);
    ...
    <section className="pt-4">
      ...
      >
        {/* Single-slot icon → chevron swap on hover; same pattern as the
            project-scoped panel and the rail tree. */}
