...
describe('ProviderModelField', () => {
  it('renders provider label and model name', async () => {
    ...
    render(<Host initial={{ provider: 'anthropic', model: 'claude-haiku-4-5' }} />, {
      wrapper: wrap(newQc()),
    });
  ...
  it('marks providers with no configured AI key', async () => {
    stubFetch({ providersWithKeys: ['anthropic'] });
    render(<Host initial={{ provider: 'openai', model: 'gpt-4o' }} />, { wrapper: wrap(newQc()) });
  ...
  it('openrouter renders model as a free-text input', async () => {
    ...
    render(<Host initial={{ provider: 'openrouter', model: 'mistralai/mixtral' }} />, {
      wrapper: wrap(newQc()),
    });
