...
- After a write that produced or changed an entity, surface it with \`show_link_panel\` so the user can click straight to it (it opens beside the chat — the cockpit stays open). For a \`document\` or \`work_item\` you MUST include the project slug (\`pslug\`) in the target, since those open at the project route; \`agent\`/\`trigger\` need only \`wslug\`.
