...
- **The `claude-code` provider — HARD-DISABLED 2026-06-02 (Phase C shake-out, `a5d0966`).** The Phase C shake-out found the cc path bypassed BOTH the C3 unattended floor (S-1) AND the agent∩caller scope ceiling (S-2): cc spawns the `claude` CLI, which re-enters via `routes/mcp.ts` (passes `callerScopes: token.scopes` = a no-op intersect; no `unattended` flag), so a trigger-fired cc library agent runs unfloored + with the agent's full scopes (not caller-bounded). **Stefan's call: cc doesn't work — hard-disable it.** The `runner.ts` preflight now refuses ANY `claude-code` run regardless of `FOLIO_CLAUDE_CODE_ENABLED`, so `ccExecute` is UNREACHABLE from both `runAgent` + `runAgentResume` → **S-1/S-2 are unreachable BY CONSTRUCTION.** The earlier "C3-CC-1 accepted residual" (cc MEDIUM-floor gap) is SUPERSEDED — the gap is no longer reachable. The provider enum stays valid (historical rows parse; they fail at preflight with `claude_code_disabled`). **REVIVAL GATE:** do NOT re-enable cc until run-derived authority (`unattended` + caller-narrowed scopes) is threaded onto the `cc-run:` minted token. Tracked as **CC-DISABLED-1** in `tasks/retro-follow-ups.md`.
