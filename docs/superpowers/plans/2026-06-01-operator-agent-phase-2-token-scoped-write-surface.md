...
> **⚠ Plan defect noted during phase-op-2 execution (shipped fix in `2a241e0`, hygiene follow-up `0b4747d`):**
> P2-8 below specifies reading `dryRun` from "the validated JSON body" for every route — but DELETE requests carry no body, so the flag was unreadable on delete paths.
> Shipped fix: a split reader — `isDryRunDelete` takes the flag from the QUERY string on DELETE; POST/PATCH read it from the validated json. Refer to commit `2a241e0` (+ `0b4747d` for the single-reader hygiene) for the actual pattern; the "validated JSON body" wording below is preserved as historical context but should not be copied for DELETE.
