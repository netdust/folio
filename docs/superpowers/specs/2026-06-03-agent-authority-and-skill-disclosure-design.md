...
> The CANONICAL, structured threat model (6 assets · 7 actors with IN/OUT markers · 14 attacks paired 1:1 with code-checkable mitigations · explicit deferrals · how-to-use) lives in the implementation plan: `docs/superpowers/plans/2026-06-03-agent-authority-and-skills.md`, section `## Threat model`. It was produced via `netdust-core:threat-modeling`. The paragraph below is the spec-level summary the plan's section expands; `/code-review` verifies against the PLAN's numbered mitigations, not this summary.
