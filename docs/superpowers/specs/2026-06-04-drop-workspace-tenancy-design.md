...
So "independent levels" stays honest: workspace-traverse ≠ see-all-projects. `listProjects` (§4.3) MUST filter by `canSeeProject` (not merely "is in a workspace I can reach"), or the traverse clause leaks the project list.

`admin` does **not** bypass grants for *visibility* (only `owner` does — user's explicit choice). `admin` retains instance-management power (mint instance tokens, manage keys, invite users) via `users.role`, but to *see* a workspace they must be invited like anyone else.
...
| **Add** `instance_skills` table | `(id PK, name UNIQUE, body text, frontmatter json, created_at)`. Seeded with the `folio` skill on boot. The `trusted` concept moves here (invariant 11 rewrite — see §6). |
...
| Surface | Change |
|---------|--------|
| `services/workspaces.ts` `listWorkspaces` | Filter by `canSeeWorkspace` (owner → all; else `workspace_access`) instead of `memberships.userId`. |
| `services/projects.ts` `listProjects` | Filter by `canSeeProject`. |
| `services/documents.ts` | Document queries currently filter by `workspace_id` as *isolation*. Re-frame: `workspace_id` is still the **scoping key** (which folder a doc is in) but no longer an isolation gate — the gate is the project/workspace access check at the route. The `__system`-union + library badging (DEAD) is removed. |
| `routes/tokens.ts` | `ceilingRole` ← `users.role`; membership lookup removed; reach-null gate ← `requireInstanceAdmin` (reworked). |
| **NEW** invitation routes | `POST/DELETE /api/v1/instance/access` (or nested): grant/revoke `workspace_access` + `project_access`. Session-only, `requireInstanceAdmin` (owner/admin can invite). Exact shape in plan. |
| **NEW/MOVED** roles route | `PATCH /api/v1/instance/users/:id/role` — set a user's instance role. Session-only, owner-gated (only owner changes roles? — §9 OQ-3). |
...
- `grantOwner` → `designateInstanceOwner` sets `users.role='owner'` (idempotent, first-wins) instead of inserting a `__system` membership.
...
- `runBootTasks` → keeps owner designation from `FOLIO_INSTANCE_OWNER`; drops the `bootstrapSystemWorkspace` call; **gains** the `instance_skills` seed + operator-agent seed at its new home.
...
- **Operator (instance-scoped):** resolved by its dedicated instance home (OQ-1), not by a workspace query. Its reach is instance-wide *by design*, still clamped by caller authority.
...
| **11** | Skill trust (`setSkillTrust`, `__system` skills page) | **Re-pointed, NOT simplified away.** Skills move from `__system` documents to `instance_skills`; the `trusted` flag + `canBlessSkill` + `setSkillTrust` + `stripManagedSkillTrust` machinery is re-pointed at `instance_skills` and **kept**. **`trusted` is capability-trust, NOT tenant-isolation** — the two are orthogonal: `trusted` gates what a skill is *allowed to do* (a blessed skill loads as TRUSTED INSTRUCTIONS into an agent's system prompt; an unblessed one rides the untrusted DATA envelope). Within one team a `member` still must not be able to bless a skill that an owner-authority agent then executes. So "no cross-tenant exposure ⇒ simplify trust" is a **category error** and is explicitly rejected here. *(Revised 2026-06-04 after review: the earlier "or simplify the trust model" parenthetical conflated isolation with trust.)* If simplifying the trust model is ever wanted, it is a **separate, explicit decision** — not a rider on this tenancy teardown. The threat model + invariant-auditor must re-confirm no write surface can set `trusted` outside the sanctioned mutator after the move (T-E). The two Deliberate-exception reads tied to `__system` (loadAgentDefinition skill read, runner AI-key read) get re-pointed and re-ratified. |
...
- **T-A (residual isolation leak — and the LOST SAFETY NET):** Every query that filtered by `workspace_id` for *isolation* — does demoting it to *scoping* leak data across the team in a way that matters? Within one team, cross-member visibility is by-design; but **agent-run authority**, **token reach**, and **`/events` SSE streams** stay real boundaries and need explicit re-derivation. Enumerate each `WHERE workspace_id` and classify isolation-vs-scoping.
  **The deeper hazard (raised in review):** under multi-tenancy, a *misclassified* `workspace_id` filter that should have stayed isolation would surface as **another tenant's data appearing** — visibly wrong, caught fast by eye. After this change, the same misclassification merely shows **a teammate's data**, which looks like a *feature*, not a bug. **The human eye will no longer catch a widened boundary.** Enumeration alone is necessary but NOT sufficient — it can't fail loudly. Therefore each of the three boundaries that remain real (**agent-run authority, token reach, `/events` scope**) MUST be pinned by an **explicit regression test that breaks if the boundary widens** (e.g. "user with no grant to ws B receives zero events from ws B's stream"; "a member-minted token cannot reach a project the member lacks"; "an agent run cannot act outside its `agent ∩ token ∩ caller` ceiling"). These tests are a **hard requirement of the relevant sequence steps (§10)**, not optional coverage — they are the *replacement* for the isolation safety net the multi-tenant model gave for free. See §8.1.
...
|---|---|---|
| **`/events` SSE scope** | A user with **no grant** to workspace B subscribes to B's `/events` (or the post-refactor equivalent) and receives **zero** B events. Widening (events leak to a non-granted member) fails the test. | 3 |
...
| **Visibility (grant)** | A `member` with `workspace_access` to A but not B gets A in `listWorkspaces` and **404/403 on B**; a `project_access`-only user can traverse the ws shell but `listProjects` returns **only** their granted project (the §3 traverse boundary). | 3 |
...
**OQ-1 — Operator agent's new home.** The operator survives (D8) and is the *only* agent needing an instance home (D9 — custom agents stay project-scoped). The real fork is **"does the operator need a `documents` row at all?"** — because its prompt and tools are code constants (`OPERATOR_PROMPT`, `OPERATOR_TOOLS`), it is unique, and it is never user-edited. Candidates, best to worst:
  ...
  Recommendation: **(d)** if we accept one operator-special-case branch in agent resolution; else **(a)**. **Avoid (b).** **User input needed** — this is the one materially-open design fork. *(Revised 2026-06-04 after review: the original (b) recommendation fought §1's own rule and made isolation enumeration load-bearing for correctness.)*
...
**OQ-3 — Who can change roles / invite?** Owner only, or owner + admin? (§4.3 roles route + invitation route gating.) Brainstorming said admins manage the instance but need invites to *see* workspaces — does "manage" include granting *others* access / changing roles? Default proposal: **owner + admin can invite; only owner can change roles.** Confirm.
