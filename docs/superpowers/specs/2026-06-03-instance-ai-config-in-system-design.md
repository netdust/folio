...
5. **Migration (trivial — one-time, local, empty table).** The local DB holds ZERO `ai_keys` rows (verified), and this runs once pre-production. So the migration is a **pure schema change**: rebuild `ai_keys` WITHOUT `workspace_id`, with unique `(provider, label)`. No consolidation, no validation, no network, no admin-prompt — there is nothing to migrate. **Guard: FAIL LOUD, do not silently resolve.** The whole simplification rests on the "zero rows" premise. If that premise is false when the migration runs (a stray staging DB, a second local with keys), the assumption broke — so `if (count(ai_keys) > 0) throw` with a clear message + log the offending rows (provider, label, source workspace), and ABORT. Surfacing a broken premise is correct + cheaper than a collision resolver; silently applying newest-wins would re-introduce the exact unsafe behavior removed for the real case. (CORRECTED twice: from validate/surface-to-admin → to fail-loud-abort. No heuristic.)
...
- the AI-key CRUD (currently `settings.ts:31-32, 111-114, 135-136` — GET list / POST upsert / DELETE). DROP the `workspace_id` predicate; re-gate from per-workspace membership to `requireInstanceAdmin`. GET list is admin-only too — instance config, not per-workspace data. **MOVE the routes off `:workspaceId`** to an instance path (e.g. `/api/v1/system/ai-keys`) so the URL matches the model. A vestigial `:workspaceId` on a route whose gate is `requireInstanceAdmin` and whose query ignores workspace is the URL-level version of the sentinel column we deleted — it lies about scope and invites a future dev to "restore" the missing workspace filter as a bug. Make-it-tell-the-truth: the path carries no workspace. (Mount likely moves from `wScope` to a `v1`-level system route, like `instance-tokens`.)
...
- **The decrypted AI key material** — the libsodium-encrypted `encrypted_key` in the `ai_keys` table (a workspace-independent instance credential row, identified by `(provider, label)` — NOT a workspace-scoped row), decryptable only with `FOLIO_MASTER_KEY`. Must never reach a worker token, a tool result, an API response, frontmatter, or a log.
...
4. **Non-admin reads/writes the key store.** → The re-scoped AI-key routes gate on `requireInstanceAdmin` — session user who is owner/admin of `__system`. A normal workspace owner is refused (403). The GET list is admin-only too (instance-level config, not per-workspace data). **DEPENDENCY VERIFIED:** `requireInstanceAdmin` is MERGED on `main` (`system-workspace.ts:282`), correct (owner/admin-of-`__system` → role, else 403), and already in production use (`tokens.ts:75` A7, `instance-tokens.ts:38,53` A12) — this mitigation stands on shipped, exercised code, not a planned dependency (verified 2026-06-03).
...
8. **Denial-of-wallet / shared-credential abuse (NEW — introduced by consolidation).** All agents now draw on the shared `__system` keys, so a prompt-injected worker in ANY workspace can exhaust rate limits or run up spend across the whole instance (the per-workspace blast radius is gone). A CONSEQUENCE of consolidation, not a reason against it. → **DECIDED: metered residual, caps deferred** (see "Settled decisions"). IN-PHASE: (a) **per-workspace usage metering** at the provider-call boundary — record, don't enforce — so the residual is detectable + abuse attributable; (b) **fail-loud trigger** on paid (non-Ollama) `__system` key creation: warn "denial-of-wallet residual now live; caps not built." DEFERRED: per-key caps + enforcement (their own phase, the day a paid key appears). Zero current exposure (Ollama free, no production) → caps now = defense ahead of need. The worker "any key, no allow-list" choice (attack 5) is why metering is mandatory: any worker can reach the expensive key, so usage must at least be visible.
...
## Settled decisions (were open; now locked for the plan)

- **Frontmatter reference key:** `ai_key_label`, default `'default'`. The resolver looks up `(provider, ai_key_label)`. Admin + worker agents reference identically. `label` is load-bearing (multi-key-per-provider allowed).
- **Migration mechanic:** use the **table-rebuild pattern** (like `0006`/`0022`), NOT native `DROP COLUMN`. The table is empty so the rebuild is free, it matches the existing migration style, and it sidesteps any "does this `bun:sqlite` version support DROP COLUMN" question. Consistency at zero cost. (The fail-loud guard runs first: `if count(ai_keys) > 0 → throw + log + abort` before the rebuild.)
- **Denial-of-wallet (attack 8): METERED RESIDUAL, caps NOT built this phase.** No paid `__system` key exists today (Ollama is free), no production, no live untrusted-content path → caps would defend a threat with zero current exposure (YAGNI). Deferred *disciplined*, on two conditions that ARE in-phase:
  1. **Build the metering now** (not optional): per-workspace usage attribution at the provider-call boundary — RECORD usage, do not enforce. Light to build; it's what makes the residual *detectable* when it goes live. (Deferring caps WITHOUT metering = can't even see the abuse = the bad deferral.)
  2. **Fail-loud trigger:** on creation of a PAID (non-Ollama) `__system` key, surface a warning — "denial-of-wallet residual is now live; per-key caps are not built." The residual goes live the moment a paid key lands; the warning makes the dormant assumption announce itself when its precondition becomes true (same instinct as the migration guard). Caps become their own small phase the day a paid key appears.
