...
> **Numbering vs executed reality (2026-06-16 note).** The phase numbers in this doc are a *historical ledger*, not a strict execution order. In particular, phases **1.5–1.9.1** are numbered as Phase-1 substeps but were actually executed *after* the Phase 2 agent backbone landed — they are operational-UI polish layers that built on top of the agent spine, not work that preceded it. This is a layering inconsistency, not a true collision; **do not renumber any executed phase** (cross-references throughout the repo depend on the existing numbers). For the canonical *live* roadmap and current state, treat **`memory/STATE.md`** as authoritative; PHASES.md is the historical phase ledger.
