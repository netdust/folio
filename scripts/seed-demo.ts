...
async function api(
  ...
  const res = await fetch(`${API}${path}`, {
    method,
    headers: {
      ...(body !== undefined && !headers['Content-Type']
...
async function createWorkItem(wslug: string, pslug: string, item: WorkItem): Promise<string> {
  const fm = item.status ? { ...(item.frontmatter ?? {}), status: item.status } : item.frontmatter;
...
const PROJECTS = [
  {
    ...
    workItems: [
      {
        title: 'Implement BYOK encryption for AI keys',
        status: 'in_progress',
        frontmatter: {
          priority: 'high',
        ...
        body: 'libsodium-encrypted at rest with server master secret from `FOLIO_MASTER_KEY`.\n\n## Acceptance\n\n- [ ] Anthropic key storage\n- [ ] OpenAI key storage\n- [x] Ollama (no key needed) passthrough',
      },
      {
        title: 'Wire MCP server endpoint',
        status: 'todo',
        frontmatter: { priority: 'high', labels: ['agents', 'phase-2'], due_date: '2026-06-15' },
        body: 'Agents are first-class users. Need a documented MCP endpoint with scoped token auth.\n\n```ts\nconst server = new McpServer({ name: "folio", version: "1.0.0" });\nserver.tool("create-document", schema, handler);\n```',
      },
      {
        title: 'Cmd-K palette: add "Switch theme" command',
        status: 'done',
        frontmatter: { priority: 'low', assignee: 'stefan@netdust.be', labels: ['ux'] },
        body: 'Done — shipped in phase-1.5.',
      },
      {
        title: 'Kanban drag-and-drop status update',
        status: 'done',
        frontmatter: { priority: 'high', labels: ['phase-1'] },
      },
      {
        title: 'Slideover Alt+M raw markdown toggle',
        status: 'done',
        frontmatter: { priority: 'medium', labels: ['ux', 'phase-1.5'] },
      },
      {
        title: 'Filter chip popover fix (forwardRef)',
        status: 'done',
        frontmatter: { priority: 'high', labels: ['bugfix', 'phase-1.5'] },
        body: "ChipAdd was a plain function component, not a forwardRef. Radix's `<PopoverTrigger asChild>` couldn't attach its ref.",
      },
      {
        title: 'sqlite-fts5 search (Phase 1.1)',
        status: 'backlog',
        frontmatter: {
          priority: 'medium',
          labels: ['phase-1.1', 'search'],
          due_date: '2026-07-01',
        },
        body: '> Out of scope for v1. Track for 1.1.\n\nFull-text search with snippets.',
      },
      {
        title: 'Email magic-link verification flow',
        status: 'backlog',
        frontmatter: { priority: 'low', labels: ['auth'] },
      },
      {
        title: 'Document the REST API in docs/API.md',
        status: 'todo',
        frontmatter: {
          priority: 'medium',
          assignee: 'stefan@netdust.be',
        ...
        body: 'Write as you build. Every route, every error code.',
      },
      {
        title: 'Optimistic UI rollback on 4xx',
        status: 'in_progress',
        frontmatter: { priority: 'high', labels: ['ux'], due_date: '2026-05-30' },
        body: 'Mutations update the UI immediately. Failures roll back with a toast.',
      },
      {
        title: 'Wire 10 skipped manual-QA scenarios',
        status: 'done',
        frontmatter: { priority: 'medium', labels: ['testing', 'phase-1.5'] },
        body: 'All 13 scenarios now have real Playwright coverage. No more `test.skip`.',
      },
    ],
    pages: [
      {
  ...
  },
  {
    ...
    workItems: [
      {
        title: 'Migrate LMS to LearnDash 4.x',
        status: 'in_progress',
        frontmatter: {
          priority: 'high',
          assignee: 'stefan@netdust.be',
          labels: ['lms', 'migration'],
          due_date: '2026-06-20',
        },
        body: 'Bedrock + LearnDash. Test on DDEV before staging.',
      },
      {
        title: 'FluentCRM segmentation by course progress',
        status: 'todo',
        frontmatter: { priority: 'medium', labels: ['crm', 'integration'] },
        body: 'Auto-tag users when they complete a course.',
      },
      {
        title: 'Add Vue.js cohort SCORM tracking',
        status: 'in_progress',
        frontmatter: {
          priority: 'medium',
          assignee: 'stefan@netdust.be',
      ...
      {
        title: 'Fix Redis cache invalidation for course lookups',
        status: 'done',
        frontmatter: { priority: 'high', labels: ['bugfix', 'caching'] },
        body: '> Never flush Redis globally — LMS cache exclusions get destroyed by `wp cache flush`. Surgical invalidation only.',
      },
      {
        title: 'Quarterly invoice batch automation',
        status: 'todo',
        frontmatter: {
          priority: 'high',
          assignee: 'stefan@netdust.be',
      ...
      {
        title: 'Domain renewal: stride.be',
        status: 'backlog',
        frontmatter: { priority: 'low', due_date: '2026-08-01', labels: ['ops'] },
      },
      {
        title: 'Replace ntdst-assistant with new build',
        status: 'in_progress',
        frontmatter: { priority: 'medium', labels: ['internal-tool'] },
      },
      {
        title: 'Pre-launch security audit',
        status: 'todo',
        frontmatter: { priority: 'high', labels: ['security'], due_date: '2026-06-25' },
      },
      {
        title: 'Course catalog page redesign',
        status: 'done',
        frontmatter: { priority: 'medium', labels: ['design', 'ux'] },
      },
      {
        title: 'Student progress dashboard',
        status: 'backlog',
        frontmatter: { priority: 'medium', labels: ['feature'] },
      },
    ],
    pages: [
      {
    ...
    workItems: [
      {
        ...
        status: 'done',
        frontmatter: { priority: 'medium', labels: ['kickoff'], due_date: '2026-05-10' },
        body: '## Goals\n\n- Re-platform from Wix to Statamic\n- Add multilingual support (NL/FR)\n- Integrate FluentForms for inquiries\n\n## Constraints\n\n- Budget: €8k\n- Launch: end of June',
      },
      {
        title: 'Statamic Peak scaffold',
        status: 'done',
        frontmatter: { priority: 'high', labels: ['scaffolding'] },
      },
      {
        title: 'Brand assets from client',
        status: 'in_progress',
        frontmatter: {
          priority: 'high',
          assignee: 'stefan@netdust.be',
        ...
        body: '> Blocked on client delivery. Logo PSD still pending.',
      },
      {
        title: 'Homepage hero block',
        status: 'in_progress',
        frontmatter: { priority: 'high', labels: ['design', 'frontend'] },
      },
      {
        title: 'About page',
        status: 'todo',
        frontmatter: { priority: 'medium', labels: ['content'] },
      },
      {
        title: 'Contact form (FluentForms)',
        status: 'todo',
        frontmatter: { priority: 'medium', labels: ['form', 'integration'] },
      },
      {
        title: 'NL ↔ FR translation pass',
        status: 'backlog',
        frontmatter: { priority: 'medium', labels: ['i18n'], due_date: '2026-06-15' },
      },
      {
        title: 'Hosting setup on Combell',
        status: 'todo',
        frontmatter: { priority: 'medium', labels: ['ops', 'combell'] },
      },
      {
        title: 'SEO baseline (titles, OG, schema.org)',
        status: 'backlog',
        frontmatter: { priority: 'low', labels: ['seo'] },
      },
      {
        title: 'Pre-launch checklist review with client',
        status: 'backlog',
        frontmatter: { priority: 'high', labels: ['launch'], due_date: '2026-06-22' },
      },
    ],
    pages: [
      {
      ...
      {
        title: 'Sprint 1 — Discovery & design',
        body: '# Sprint 1\n\n- Kickoff call ✓\n- Design system\n- Wireframes\n- Client sign-off',
        parentSlug: 'milestones',
      },
      {
        title: 'Sprint 2 — Build & content',
        body: '# Sprint 2\n\n- Statamic Peak scaffold ✓\n- Page builder blocks\n- Content migration from Wix\n- Forms integration',
        parentSlug: 'milestones',
      },
      {
        title: 'Sprint 3 — Translation & launch',
...
async function main() {
  ...
  for (const p of PROJECTS) {
    ...
    if (!defaultTable) {
      throw new Error(
        `Project ${p.slug}: no default table was created. seedProjectDefaults must have failed.`,
      );
    }
