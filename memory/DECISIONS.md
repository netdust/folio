...
## Folio's caller-bounded authority is a do-not-regress (validated 2026-06-06 vs Multica)

Studying Multica (multica-ai/multica — a mature, years-shipped Go/Postgres agent platform) confirmed that Folio's authority model is **stronger than where a mature competitor landed**, and this is now a defended decision, not an accident.

- **Multica's** task token (`mat_`) narrows ONLY the workspace; within it the agent is **owner-equivalent** (X-User-ID = the owner's; agent-id is attribution, not a capability ceiling; the only fence is an account-billing denylist explicitly NOT applied globally — "agent and human are interchangeable" is their documented default).
- **Folio's** authority is the intersection `agent ∩ token ∩ caller` plus a separate project ceiling, fail-closed, enforced at three convergence points: `executeTool`'s double-membership check (`agent-tools.ts:329`), `intersectAgentProjects`, and `effectiveReach` (`runner.ts:513`, defaults deny-all). A Folio agent NEVER inherits caller-equivalent authority.

**Decision:** this intersection-ceiling is the differentiator a mature peer chose not to build. Do NOT regress it for convenience (e.g. "just let the operator act as the caller"). Keep `agent-tools.ts:329` and `runner.ts:513` test-covered. Any new credential/reach branch MUST declare its position relative to the caller ceiling (`effectiveReach`) and, if it can act unattended, against `UNATTENDED_FLOORED_SCOPES` (`agent-tools.ts:161`). Related: [[operator-is-an-agent-not-a-seeded-bot]], the 9f75c40 token-mint-scope-ceiling fix. Full study: `docs/superpowers/specs/2026-06-06-multica-architecture-study.md`.
