...
- **Re-seed runs DURING render, not in an effect** (`useDocumentDraft`): the slideover is mounted persistently and passes an empty fallback doc while loading; the body editors (Milkdown + CodeMirror) read `value` only at mount, so an effect-based re-seed left a stale/empty frame. Render-phase `setDraft` gated by a key ref (the React "derive state from changing prop" pattern, loop-safe). Body editors are keyed ``key={`rich-${doc.slug}-${doc.updatedAt}`}`` so they remount onto the freshly-seeded body after a load/save.
