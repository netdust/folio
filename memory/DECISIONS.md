...
- **`work_item` / `page` slugs are now IMMUTABLE** (extends the table/agent/trigger precedent). `maybeRegenerateSlug` + `isSlugAutoDerived` were REMOVED from `services/documents.ts`, and the SECOND call site in `routes/documents.ts` (markdown-PATCH path) was also neutralized. A retitle changes the title only — never the slug — so `[[slug]]` links stay valid forever. No rename cascade. Pinned by a test in `documents.test.ts` (`retitling a work_item does NOT change its slug`); if anyone re-adds regeneration, it fails loudly.
