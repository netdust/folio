...
- `bun run test` in `apps/web/` → Vitest. 127 / 127 pass + 1 skipped (jsdom limitation on Milkdown initial render).
...
- `bun run e2e` in `apps/web/` → Playwright. 10 passing (3 smoke + 4 click-through + scenarios 1, 2, 15), 10 skipped (scenarios 3, 4, 5, 6, 8, 9, 10, 11, 12, 13 — each scaffolded with a per-test TODO about which selector still needs work).
