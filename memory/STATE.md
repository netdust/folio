...
- **Phase 1.6.1 (Rail completeness):** NEXT — half-day polish, expanded after Stefan's walkthrough. Closes UI dead-ends: create-project-from-rail (today only on /w/:wslug/), create/rename/delete tables (today every project's table is stuck called "Work Items"), rename/delete views (backend supports both), and Wiki as a rail leaf. All backend already exists — UI-only phase. See `docs/PHASES.md` §1.6.1.
