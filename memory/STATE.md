...
**▶ NEXT SESSION — Session 4: M2 RunSink (2.6).** Own plan + own review cluster (the most security-critical path — audit marks it XL/High-risk, "break down before starting"). Goal: `runner.ts` 37 `ctx.sink` branches → ~0 via a `RunSink` interface (extract `DocumentRunSink`/`ConversationSink`, split `runLoop`'s per-round body into named functions, `runLoop` under ~150 lines). Inv 2/3/12. Depends on 2.5 (landed). Lean on the 83 existing runner tests as the behavior-preservation net. THEN Session 5 = M3 polish (docs incl. API.md AI-keys path = M3.1, pagination, typing, the 7 Playwright IOUs). Start with `harnessed-development`.
