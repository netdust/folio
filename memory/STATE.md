...
> 1. **C-7** — rename `lib/mcp-dispatch.ts`/`executeMcpTool`/`McpAuthContext` → `lib/agent-tools.ts`/`executeTool(token, actor, name, args)`/plain `{token, actor}`. Inside-agent === outside-agent, one auth model, runner is NOT an MCP client. PLUS decide deliberately: skeleton-`__echo`-now (real tools in D-3) vs. pull the real `TOOLS` extraction forward — the former means the "set up a project for me" demo can't work until Sub-phase D.
